<template>
  <div class="img-container" ref="svgContainer">
    <svg
      ...
      :width="img.width * scaleRect"
      :height="img.height * scaleRect"
    ...
    >
      ...
      <rect
        ...
        :x="item.x * scaleRect"
        :y="item.y * scaleRect"
        :width="item.w * scaleRect"
        :height="item.h * scaleRect"
      ...
      <v-editpoint
        ...
        @funcStart="changeRectPointStart"
        @funcEnd="changeRectPointEnd"
        :scaleRect="scaleRect"
      />
    </svg>
  </div>
...
export default {
  ...
  created() {
    // 获取其他组件传来的信息
    vm.$on("addRects", (rects, height, width, color) => {
      this.rects = rects;
      if (rects.length > 0) {
        var i = 0;
        for (; i < rects.length; i++) {
          this.fixPointsHandle(i);
        }
        this.svgColor = color;
      } else {
        this.fixPoints = [];
      ...
      this.scaleRect = height < width ? 512 / width : 512 / height;
    });
    vm.$on("deleteRect", () => {
      this.deleteRect();
    });
  ...
  mounted() {
    ...
    svgElem.addEventListener("mousewheel", (event) => {
      ...
      if (event.wheelDeltaY > 0) {
        ...
        event.wheelDeltaY < 0 &&
        this.img.width * this.scaleRect > 320
      ) {
        this.scaleRect -= 0.1;
        // this.svgChangeHandle()
      }
  ...
  methods: {
    ...
    fixPointsHandle(index) {
      ...
      let centerTop = {
        fixedX: pointData.x + pointData.w / 2,
        fixedY: pointData.y,
      };
      let rightTop = { fixedX: pointData.x + pointData.w, fixedY: pointData.y };
      let centerLeft = {
        fixedX: pointData.x,
        fixedY: pointData.y + pointData.h / 2,
      };
      let centerRight = {
        fixedX: pointData.x + pointData.w,
        fixedY: pointData.y + pointData.h / 2,
      };
      let leftBottom = {
        fixedX: pointData.x,
        fixedY: pointData.y + pointData.h,
      };
      let centerBottom = {
        fixedX: pointData.x + pointData.w / 2,
        fixedY: pointData.y + pointData.h,
    ...
    changeRectPoint(xx, yy) {
      ...
      if (activeFixPosition === 1) {
        ...
        fixedObj = {
          ...
          w: fixData.w,
          h:
            y <= fixData.y + fixData.h
              ? fixData.h - (y - fixData.y)
              : y - fixData.y - fixData.h,
        };
      } else if (activeFixPosition === 6) {
        // centerBottom
        fixedObj = {
          x: fixData.x,
      ...
      } else if (activeFixPosition === 3) {
        // leftCenter
        fixedObj = {
          x: x < fixData.x + fixData.w ? x : fixData.x + fixData.w,
          y: fixData.y,
          w:
            x < fixData.x + fixData.w
              ? fixData.x + fixData.w - x
              : x - (fixData.x + fixData.w),
          h: fixData.h,
        };
      } else if (activeFixPosition === 4) {
        // rightCenter
        fixedObj = {
        ...
        fixedObj = {
          ...
          h:
            y < fixData.y + fixData.h
              ? fixData.y + fixData.h - y
              : y - (fixData.y + fixData.h),
        };
      } else if (activeFixPosition === 2) {
        // rightTop
        fixedObj = {
          x: x > fixData.x ? fixData.x : x,
          ...
          h:
            y > fixData.y + fixData.h
              ? y - fixData.h - fixData.y
              : fixData.h + fixData.y - y,
        };
      } else if (activeFixPosition === 5) {
        // leftBottom
        fixedObj = {
          x: x < fixData.x + fixData.w ? x : fixData.x + fixData.w,
          y: y > fixData.y ? fixData.y : y,
    ...
    deleteRect() {
      ...
      if (index === -1) {
        ...
        // console.log(this.activeRectIndex);
